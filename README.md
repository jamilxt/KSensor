...
### Sample Android App
A minimal Android sample app is included in this repository to help you test the library quickly.

- Module: `:sample`
- What it does: Displays live Accelerometer values using `KSensor.registerSensors(...)`.
- How to run:
  - Open the project in Android Studio.
  - Select the "sample" run configuration or set the `:sample` module as the run target.
  - Run on a device/emulator with sensors (Accelerometer is usually available on devices; emulators may not provide sensor data).
- Try more sensors: Update the `types` list in `MainActivity.kt` to add other `SensorType`s. For `LOCATION`, add the required permissions to the `:sample` AndroidManifest as shown above.
